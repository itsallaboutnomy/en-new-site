...
@section('page-title',' | General Complaint')
                ...
                <div class="row no-gutters align-items-center">
                    <div class="col-lg-6 animate__animated animate__animated animate__fadeInLeft">
                        <div class="fr-left-content main-headings">
                            <h1 class="text-white h-primary text-uppercase">General Complaint</h1>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <section class="fr-forms-wrapper padd mt-5">
        <div class="content">
            <div class="container">
                ...
                <form id="general-complaint-form" method="POST" action="{{ route('app.support.generalComplaint.store') }}" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="request_type" id="request_type" value="general-complaint">
                    <div class="row my-5">
                            ...
                            <div class="form-group required">
                                ...
                                <select name="batch_id" id="batch_id" class="form-control  @error('batch_id') is-invalid @enderror">
                                    ...
                                    @foreach($trainingBatches as $trainingBatch)
                                        <option value="{{ $trainingBatch->id }}" >{{ $trainingBatch->name }}</option>
                                    @endforeach
                                </select>
                                @error('batch_id')
                                <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                        ...
                        <div class="col-lg-6 mb-4">
                            <div class="form-group required">
                                <label for="attachment" class="label">Attach Image</label>
...
@section('scripts')
    ...
    <script>
        ...
        $("#general-complaint-form").validate({
