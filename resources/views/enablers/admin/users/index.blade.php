                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <div class="card-tools">
                            <a class="btn btn-block btn-primary btn-sm" href="{{ route('users.create') }}"><i class="fas fa-plus-circle"></i> Add New User</a>
                        </div>
                    </div>
                            ...
                            @forelse ($users as $user)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        ...
                                        <form id="user-status-{{ $user->id }}" action="{{ route('users.update-status',$user->id) }}" method="POST" style="display: none;">@csrf</form>

                                        <a class="btn btn-xs btn-info" href="{{ route('users.edit',$user->id) }}"><i class="far fa-eye"></i> Edit</a>

                                        <a class="btn btn-xs btn-danger"  href="javascript:void(0);"  onclick="removeRecord('{{ $user->id }}');"><i class="fas fa-trash-alt"></i> Delete</a>
                                        <form id="user-delete-{{ $user->id }}" action="{{ route('users.destroy',$user->id) }}" method="POST" style="display: none;">@csrf @method('DELETE')</form>
                                    </td>
                                </tr>
                            @empty
                                <tr>
